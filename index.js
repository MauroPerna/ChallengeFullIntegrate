...
function devolverCantidadClicks(clicks) {
	//Aca debes realizar la logica
	if(clicks.length < 1) throw new Error("Sorry, something went wrong :(")
	let objClick = {}
	clicks.forEach((e) => {
		let arr = e.split(/[\,\.]/g)
		let numberOfClicks = parseInt(arr[0]);
		let hightLevelDomain = arr[arr.length - 1]; // .com, .uk, .org
		let domain = arr.slice(arr.length -2, arr.length).join('.'); // google.com
		let subdomain = arr.slice(1, arr.length).join('.'); // mail.yahoo.com
		let aux = subdomain.split('.'); // ['mobile', 'sports', 'yahoo', 'com']
